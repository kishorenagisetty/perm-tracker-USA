...
async function scrapeImmilane() {
  ...
  const data = await page.evaluate(() => {
    ...
    return {
      ...
      netChange: match(/Net change\s+([−\-]?[\d,]+)/),
      lastMonth: match(/Last month \(([^)]+)\)/),
      certifiedAvg: match(/Certified cases avg\s+([\d,]+ days)/),
      rfiAvg: match(/RFI cases avg\s+([\d,]+ days)/),
      deniedAvg: match(/Denied cases avg\s+([\d,]+ days)/),
...
function buildEmail(d) {
  ...
  <div class="note">Auto-sent every 7 days | Source: immilane.com</div></div>
  </div></body></html>`;
}
